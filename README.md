# MGERT2
re-work of [MGERT](https://github.com/andrewgull/MGERT) - aka a big-ass 6 year old python script

Before starting to work, check the Issues tab and Project requirements `docs/requirements.md`

`dev`branch is protected, you need to create a new branch to contribute to the project.

Follow [this guide](https://gist.github.com/joshbuchea/6f47e86d2510bce28f8e7f42ae84c716) to write better commit messages.